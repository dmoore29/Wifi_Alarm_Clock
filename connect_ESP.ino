boolean connect_ESP(){//returns 1 if successful or 0 if not
  ...
  Serial.println(F("CONNECTED"));//yay, connected
  ...
  if(read_until_ESP(keyword_carrot,sizeof(keyword_carrot),5000,0)){//go wait for the '>' character, ESP ready for the payload
    ...
    Serial.println(F("SENT"));//yay, it was sent
  ...
  Serial.println(F("FAILED TO GET >"));
  
  }//First OK
  else{
  Serial.println(F("FAILED TO CONNECT"));//something went wrong
  setup_ESP();//optional, this will go setup the module and attimet to repair itself - connect to SSID, set the CIPMUX, etc...
  }
